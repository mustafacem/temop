...
from proposal_droid.whisper_speech_to_text.whisper_speech_to_text import transcribe_audio
...
def create_docx(items_dict):
    ...
    if 'Use Case Description' in items_dict and items_dict['Use Case Description']:
        doc.add_paragraph(items_dict['Use Case Description'])
    for key, value in items_dict.items():
        if key != 'Use Case Description' and value:
...
def main():
    ...
    elif st.session_state.current_item_index < len(st.session_state.items_dict.keys()) - 1:
    ...
    if selected_tab == "Upload Notes":
        file_type = st.selectbox("Select the type of notes you want to upload:", 
                                 ["Image", "Voice Recording", "YouTube Link", "Website Link", "Plain Notes"])
        uploaded_file = None
        ...
        if file_type == "Image" and uploaded_file is not None:
            ...
            if website_link and st.button("Extract Text from Website"):
                ...
                st.session_state.notes_finalized = False
                st.rerun()
        elif file_type == "Plain Notes":
            plain_text = st.text_area("Enter your plain notes here:")
            if st.button("Submit Plain Notes"):
                st.session_state.notes = plain_text
                st.session_state.notes_finalized = False
                st.success("Plain notes saved!")
                st.rerun()
    ...
    # ---------------- Tab 3: Generate Use Case ----------------
        ...
        if st.session_state.notes_finalized:
            # If no use case generated yet, generate one.
            if not st.session_state.use_case_description:
                st.session_state.use_case_description = ask_chatgpt(
                    "Generate a very short use case description from the following notes: " + st.session_state.notes
                )
            # Always recalc AI opinion for use case description.
            opinion_use_case = checker(st.session_state.use_case_description, st.session_state.use_case_description)
            st.subheader("Use Case Description")
            st.write("**AI Opinion for Use Case Description:**")
            st.write(opinion_use_case)
            st.write(st.session_state.use_case_description)
            action_uc = st.selectbox("Select action for Use Case Description:", ["Accept", "Edit", "Add"], key="action_use_case")
            if action_uc == "Accept":
                if st.button("Confirm Use Case Description", key="confirm_use_case"):
                    ...
                    st.success("Use case description accepted!")
                    st.rerun()
            elif action_uc == "Edit":
                edited_uc = st.text_area("Edit Use Case Description:", st.session_state.use_case_description, key="edit_use_case")
                if st.button("Save Use Case Description", key="save_use_case"):
                    st.session_state.use_case_description = edited_uc
                    st.success("Use case description updated!")
                    st.rerun()
            elif action_uc == "Add":
                additional_uc = st.text_area("Add to Use Case Description:", key="add_use_case")
                if st.button("Add to Use Case Description", key="add_confirm_use_case"):
                    prompt_for_uc = (
                        "Expand the use case description based on the following additional information: "
                        + additional_uc
                        + " and the current use case: "
                        + st.session_state.use_case_description
                    )
                    new_uc = ask_chatgpt(prompt_for_uc)
                    st.session_state.use_case_description += "\nAdditional content:\n" + new_uc
                    st.success("Additional content added to use case description!")
                    st.rerun()
        else:
    ...
    # ---------------- Tab 4: Proposal Items ----------------
        ...
        if st.session_state.use_case_finalized:
            ...
            if st.session_state.current_item_index < len(proposal_keys):
                ...
                # Generate proposal item if not already generated.
                if st.session_state.items_dict[current_key] is None:
                    prompt = (
                        f"Generate a short 2-4 sentence {current_key} for a business proposal based on the following use case description: "
                        f"{st.session_state.use_case_description}"
                    )
                    generated_text = ask_chatgpt(prompt)
                    st.session_state.items_dict[current_key] = generated_text
                # Always recalc AI opinion for the proposal item.
                opinion_ai = checker(st.session_state.items_dict[current_key], st.session_state.items_dict[current_key])
                st.write(f"**{current_key}:**")
                st.write(st.session_state.items_dict[current_key])
                st.write("**AI Opinion for {}:**".format(current_key))
                st.write(opinion_ai)
                action = st.selectbox(f"Select action for {current_key}:", ["Accept", "Edit", "Add"], key=f"action_{current_key}")
                if action == "Accept":
                    if st.button(f"Confirm {current_key}", key=f"confirm_{current_key}"):
                        st.session_state.current_item_index += 1
                        st.success(f"{current_key} accepted!")
                        st.rerun()
                elif action == "Edit":
                    edited_value = st.text_area(f"Edit {current_key}:", st.session_state.items_dict[current_key], key=f"edit_{current_key}")
                    ...
                    if st.button(f"Add to {current_key}", key=f"add_confirm_{current_key}"):
                        prompt_for_reg = (
                            f"Expand the {current_key} for the business proposal based on the following additional information: "
                            f"{additional_value} and the use case description: {st.session_state.use_case_description}"
                        )
                        new_value = ask_chatgpt(prompt_for_reg)
                        st.session_state.items_dict[current_key] += f"\nAdditional content:\n{new_value}"
                        st.success(f"Additional content added to {current_key}!")
                        st.rerun()
            else:
                st.success("All proposal items processed! Proceed to the 'Download' tab.")
        else:
